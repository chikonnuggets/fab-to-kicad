...
class IPC2581Parser:
    ...
    def _parse_nets(self):
        ...
        # IPC-2581A/B: explicit Net/LogicalNet elements
        ...
        # IPC-2581C: nets appear as net="..." attributes on Set elements inside LayerFeature
    ...
    def _parse_padstacks(self):
        """Parse padstack definitions — handles both IPC-2581A/B (Padstack) and C (PadStackDef)."""

        # Build primitive size dictionary from DictionaryStandard
        prim_map = {}  # id -> (shape, w, h)
        for entry in self.root.iter("EntryStandard"):
            eid = entry.get("id", "")
            for child in entry:
                tag = child.tag
                if tag == "Circle":
                    d = self._float(child.get("diameter", 0))
                    prim_map[eid] = ("circle", d, d)
                elif tag in ("Rect", "Rectangle"):
                    w = self._float(child.get("width", 0))
                    h = self._float(child.get("height", w))
                    prim_map[eid] = ("rect", w, h)
                elif tag == "RectRound":
                    w = self._float(child.get("width", 0))
                    h = self._float(child.get("height", w))
                    prim_map[eid] = ("roundrect", w, h)
                elif tag == "Oval":
                    w = self._float(child.get("width", 0))
                    h = self._float(child.get("height", w))
                    prim_map[eid] = ("oval", w, h)

        # IPC-2581C: PadStackDef
        for ps in self.root.iter("PadStackDef"):
            ps_id = ps.get("name", "")
            if not ps_id: continue
            info = {"shape": "circle", "width": 0.8, "height": 0.8, "drill": 0.0, "type": "smd"}

            hole = ps.find("PadstackHoleDef")
            if hole is not None:
                drill = self._float(hole.get("diameter", 0))
                info["drill"] = drill
                if drill > 0:
                    info["type"] = "thru_hole"

            for ppd in ps.findall("PadstackPadDef"):
                spr = ppd.find("StandardPrimitiveRef")
                if spr is not None:
                    ref_id = spr.get("id", "")
                    prim = prim_map.get(ref_id)
                    if prim:
                        info["shape"], info["width"], info["height"] = prim
                        break

            self.padstack_map[ps_id] = info

        # IPC-2581A/B: Padstack
        for ps in self.root.iter("Padstack"):
            ps_id = ps.get("id", ps.get("name", ""))
            if not ps_id or ps_id in self.padstack_map: continue
    ...
    def _parse_components(self):
        ...
        for cmp in self.root.iter("Component"):
            ...
            side_raw = cmp.get("mountType", cmp.get("layerRef", cmp.get("side", "TOP"))).upper()
            side = "B" if "BOT" in side_raw or side_raw == "B" or "B.CU" in side_raw else "F"

            # IPC-2581C: x/y in child Location element, rotation in Xform element
            loc = cmp.find("Location")
            xform = cmp.find("Xform")
            if loc is not None:
                x = self._float(loc.get("x"))
                y = -self._float(loc.get("y"))  # flip Y
                rot = self._float(xform.get("rotation")) if xform is not None else 0.0
            else:
                # IPC-2581A/B: x/y/rotation as attributes
    ...
    def _parse_board_outline(self):
        """Parse board outline — IPC-2581C uses Edge.Cuts LayerFeature with Polyline."""
        # IPC-2581C: board outline is in LayerFeature layerRef="Edge.Cuts"
        for lf in self.root.iter("LayerFeature"):
            if lf.get("layerRef", "").upper() in ("EDGE.CUTS", "OUTLINE", "BOARD"):
                pts = []
                for poly in lf.iter("Polyline"):
                    pb = poly.find("PolyBegin")
                    if pb is not None:
                        pts.append((self._float(pb.get("x")), -self._float(pb.get("y"))))
                    for step in poly.iter("PolyStepSegment"):
                        pts.append((self._float(step.get("x")), -self._float(step.get("y"))))
                if pts:
                    self.outline.points = pts
                    return
                # Fallback: Line elements on Edge.Cuts
                for s in lf.iter("Set"):
                    for us in s.iter("UserSpecial"):
                        for line in us.findall("Line"):
                            x1 = self._float(line.get("startX")); y1 = -self._float(line.get("startY"))
                            x2 = self._float(line.get("endX"));   y2 = -self._float(line.get("endY"))
                            self.outline.points.extend([(x1, y1), (x2, y2)])
                if self.outline.points:
                    return

        # IPC-2581A/B fallbacks
        ...
        for bb in self.root.iter("BoundingBox"):
            x = self._float(bb.get("x")); y = -self._float(bb.get("y"))
    ...
    def _parse_layout(self):
        """Parse copper tracks and vias from LayerFeature elements."""
        for lf in self.root.iter("LayerFeature"):
            layer_ref = lf.get("layerRef", lf.get("layer", ""))
            kicad_layer = self._kicad_layer_for_ipc(layer_ref)
            is_copper = kicad_layer.endswith(".Cu")

            for s in lf:
                net = s.get("net", "")
                pad_usage = s.get("padUsage", "")

                # IPC-2581C vias: Set[padUsage=VIA] > Pad > Location
                if pad_usage == "VIA" and is_copper:
                    for pad_el in s.findall("Pad"):
                        loc = pad_el.find("Location")
                        if loc is None: continue
                        x = self._float(loc.get("x")); y = -self._float(loc.get("y"))
                        # Get size from padstack ref
                        ps_ref = pad_el.get("padstackDefRef", "")
                        ps = self.padstack_map.get(ps_ref, {})
                        size = ps.get("width", 0.8)
                        drill = ps.get("drill", size * 0.5)
                        self.vias.append(Via(x, y, size, drill, net))
                    continue

                # IPC-2581C tracks: Set > Features > UserSpecial > Line
                feats = s.find("Features")
                if feats is not None and is_copper:
                    for us in feats.iter("UserSpecial"):
                        for line_el in us.findall("Line"):
                            x1 = self._float(line_el.get("startX")); y1 = -self._float(line_el.get("startY"))
                            x2 = self._float(line_el.get("endX"));   y2 = -self._float(line_el.get("endY"))
                            w  = self._float(line_el.get("lineWidth", "0.25"))
                            self.tracks.append(Track(x1, y1, x2, y2, w, kicad_layer, net))

            # IPC-2581A/B: direct Line and Conductor children
            for line_el in lf.findall("Line"):
                net = line_el.get("net", "")
                w   = self._float(line_el.get("lineWidth", line_el.get("width", 0.25)))
                x1  = self._float(line_el.get("x1")); y1 = -self._float(line_el.get("y1"))
                x2  = self._float(line_el.get("x2")); y2 = -self._float(line_el.get("y2"))
                self.tracks.append(Track(x1, y1, x2, y2, w, kicad_layer, net))

            for seg in lf.iter("Conductor"):
                net = seg.get("net", "")
                w   = self._float(seg.get("lineWidth", seg.get("width", 0.25)))
                for line_el in seg.findall("Line"):
                    x1 = self._float(line_el.get("x1")); y1 = -self._float(line_el.get("y1"))
                    x2 = self._float(line_el.get("x2")); y2 = -self._float(line_el.get("y2"))
                    self.tracks.append(Track(x1, y1, x2, y2, w, kicad_layer, net))

        # IPC-2581A/B standalone Via elements
        for via_el in self.root.iter("Via"):
            x    = self._float(via_el.get("x")); y = -self._float(via_el.get("y"))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2:
        ...
        output_file = os.path.join(out_dir, base + "_IPC.kicad_pcb")
